...
\makeatletter
\expandafter\def\csname r@eq:projection\endcsname{{33}{5}{}{equation.33}{}}
\expandafter\def\csname r@eq:lemma1\endcsname{{36}{5}{}{equation.36}{}}
\expandafter\def\csname r@eq:lemma2\endcsname{{38}{5}{}{equation.38}{}}
\expandafter\def\csname r@eq:delta_step\endcsname{{42}{5}{}{equation.42}{}}
\expandafter\def\csname r@eq:state_step\endcsname{{43}{5}{}{equation.43}{}}
...
backlog, communication service, mobility delay, and cross domain threat propagation .  The central idea is a monotone propagation model and a risk
...
risk bound is derived under explicit monotonicity and Lipschitz assumptions.
Experiments on public city scale mobility records indicate improved stressed
...
networks, and crew routing through explicit decisions and completion events}\DIFaddend .
\end{abstract}
...
patterns and raises the load on communication assisted charging services }%DIFDELCMD < \cite{old-cui2021pricing,ran2024fast}%%%
...
represent charging service, backlog, communication capacity, feeder margin, and
mobilitydelay.  The digital twin is driven by a monotone threat propagation
operator over road}\DIFdelend \DIFaddbegin \DIFadd{compact graph recurrent model forecasts station arrivals and
charging energy.  A nonnegative transition model propagates mobility}\DIFaddend , power,
and communication \DIFdelbegin \DIFdel{threat fields.  Instead of
...
separate, which makes the role of each part explicit in the analysis and in the experiments}\DIFdelend \DIFaddbegin \DIFadd{threats over a finite horizon.  Central PC evaluates each
...
the spectral growth of the threat propagation operator, the action sensitivity
...
estimation.  Later studies considered uncertainty in charging station planning, coordinated urban power and transportation networks , and extreme fast charging facilities }%DIFDELCMD < \cite{deb2022charging,li2023milp,qian2021fast}%%%
...
\DIFdelbegin \DIFdel{and grid states }%DIFDELCMD < \cite{old-cui2021pricing,li2024inverse,ran2024fast}%%%
...
%DIFDELCMD < %%%
\DIFdel{The restoration problem considered here draws from both lines of work.  The
...
view from interdependent infrastructure analysis}\DIFdelend \DIFaddbegin \DIFadd{threat propagation, feeder
...
several intervals.  A continuous simulator can rank candidate actions}\DIFaddend , while
the \DIFdelbegin \DIFdel{cost evaluation
...
action is applied.  }\DIFdelend \DIFaddbegin \DIFadd{realized state must wait for the discrete event that makes the action
...
\DIFaddbegin \DIFadd{Comparing a proposed decision with a common reference also has an established
...
adverse model choice }\cite{petrik2016baseline}\DIFadd{.  That work studies discounted
...
\DIFaddend \subsection{Graph forecasting for charging states}
...
}%DIFDELCMD < \cite{old-deng2021stnorm,old-jiang2023staeformer,old-zhang2023megacrn}%%%
...
\DIFaddbegin \DIFadd{Here $b^{\pi}=b^{\rm cen}$ for central PC candidate construction and
...
\end{algorithm}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Reference protection compares a proposed route with an executable alternative
...
\DIFadd{\Delta(P,M)}&\DIFadd{=\widehat Q_t(P,M)-\widehat Q_t(P_b,M),}\\
\DIFadd{U(P)}&\DIFadd{=\max_{M\in\mathcal U_V}\Delta(P,M).
\label{eq:reference-difference}
}\end{align}
\DIFadd{The subtraction in \eqref{eq:reference-difference} uses the same matrix for
...
\DIFadd{\tau}&\DIFadd{=10^{-10}\max\{1,\max_{P,M}|\widehat Q_t(P,M)|\}.
}\end{align}
\DIFadd{For a nonnegative margin ratio $\nu$, PC with reference protection returns
}\begin{equation}
\DIFadd{P_g=\begin{cases}
P_u,&U(P_u)<-\mu-\tau,\\
P_b,&\text{otherwise}.
\end{cases}
\label{eq:protected-route}
}\end{equation}
\DIFadd{Thus an insufficient predicted advantage retains the forecast matched route,
...
\DIFaddbegin \DIFadd{To separate the influence of uncertain parameters from route construction,
...
\DIFadd{Travel uses geocoded station
...
\begin{algorithm}[t]\DIFnewtablecolor
...
\DIFaddend Algorithm~\ref{alg:pcr} states the decision rule.  Forecasting, propagation,
...
The analysis \DIFdelbegin \DIFdel{uses the Euclidean norm.  The projection onto a closed convex set
is nonexpansive, so
}\begin{displaymath}
\DIFdel{\|\Pi_{\mathcal{Z}}(u)-\Pi_{\mathcal{Z}}(v)\|\le \|u-v\|.
%DIFDELCMD < %%%
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\textit{\DIFdel{Assumption 1.}}  %DIFAUXCMD
\DIFdel{For all feasible actions, the stress map $d(x,a)$ is
Lipschitz in $x$ with constant $L_d$, and
the one step physical state transition
$F(x,a,\zeta)$ is Lipschitz in $(x,\zeta)$ with constants $L_x$ }\DIFdelend \DIFaddbegin \DIFadd{separates continuous prediction, finite route selection, }\DIFaddend and
\DIFdelbegin \DIFdel{$L_z$.  }\begin{displaymath}
...
%DIFDELCMD < %%%
\textit{\DIFdel{Assumption 2.}}  %DIFAUXCMD
\DIFdel{The stage risk in \eqref{eq:stage} is Lipschitz in
$(x,\zeta)$ with constants $C_x$ and $C_z$ for all feasible actions.
}\begin{displaymath}
\DIFdel{|L(x,a)-L(\tilde x,a)|
\le C_x\|x-\tilde x\|+C_z\|\zeta-\tilde \zeta\|.
}\end{displaymath}%DIFAUXCMD
\DIFdel{The assumption holds for the implemented model because every service equation
is a composition of affine maps, minimum operators, positive part operators, and
bounded products on the compact feasible set}\DIFdelend \DIFaddbegin \DIFadd{executed service.  Continuous sensitivity does not cover packet deadlines,
...
arithmetic.  Electrical feasibility refers to the stated numerical criteria}\DIFaddend .

\DIFdelbegin \textit{\DIFdel{Lemma 2.}}  %DIFAUXCMD
\DIFdel{For fixed action sequence and disturbance sequence, if two
rollout trajectories start from threat fields $\zeta_0$ and $\tilde\zeta_0$ and from the same physical state, then
}\begin{align*}
...
%DIFDELCMD < %%%
\textit{\DIFdel{Proof.}}  %DIFAUXCMD
\DIFdel{Let $\Delta_h=\|\zeta_h-\tilde\zeta_h\|$ }\DIFdelend \DIFaddbegin \DIFadd{For the continuous map, hold the forecast continuation fixed }\DIFaddend and \DIFdelbegin \DIFdel{$E_h=\|x_h-\tilde x_h\|$.
From \eqref{eq:propagation} and \eqref{eq:projection}, }\begin{align*}
\DIFdel{\Delta_{h+1}
}&\DIFdel{\le \|M(\zeta_h-\tilde\zeta_h)+D(d_h-\tilde d_h)\| \nonumber}\\
&\DIFdel{\le \|M\|\Delta_h+\|D\|L_d E_h .
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{set packet
and electrical factors to one.  With $b=e+\rho_q q$ and ungated charging
service $\bar s$, define
}\begin{equation}
\DIFadd{F_{c,h}(x,a,\zeta)=(\hat m_{h+1},\hat e_{h+1},b-\bar s).
\label{eq:continuous-service}
}\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{Applying this recursion once gives
$\Delta_1\le \|M\|\Delta_0+\|D\|L_dE_0$.
Applying it repeatedly yields
\eqref{eq:lemma1} by induction.  The base case is immediate.  If the statement
holds for $h$, substitution into the one step recursion gives the statement for
$h+1$.  This completes the proof.  }%DIFDELCMD < \hfill %%%
\DIFdel{$\square$
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textit{\DIFdel{Lemma 3.}}  %DIFAUXCMD
\DIFdel{Under Assumption 1, }\DIFdelend \DIFaddbegin \DIFadd{Let $L_c$ be \eqref{eq:stage} evaluated with the same service.
...
The derivation includes positive denominators, }\DIFaddend the \DIFdelbegin \DIFdel{physical state error satisfies
}\begin{displaymath}
\DIFdel{E_h\le
\sum_{k=0}^{h-1}L_x^{h-1-k}L_z\Delta_k .
%DIFDELCMD < %%%
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{normalized allocator,
...
\DIFdelbegin \textit{\DIFdel{Proof.}}  %DIFAUXCMD
\DIFdel{The two rollouts use the same action sequence.
Assumption 1
gives $E_{h+1}\le L_xE_h+L_z\Delta_h$.  Since the initial physical state is the
same, $E_0=0$.  Expanding the recursion gives
$E_1\le L_z\Delta_0$ and $E_2\le L_xL_z\Delta_0+L_z\Delta_1$.  Repeating the expansion gives
\eqref{eq:lemma2}.
}%DIFDELCMD < \hfill %%%
\DIFdel{$\square$
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \textit{Theorem 1.}  Let $J_H(x_0,\zeta_0)$ be the discounted $H$ step \DIFdelbegin \DIFdel{risk of }\DIFdelend \DIFaddbegin \DIFadd{sum of $L_c$ for
...
\DIFdelbegin \textit{\DIFdel{Proof.}}  %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{The complete proof in the Appendix, part~\ref{supp:theory}, propagates
the weighted state and threat error and sums its stage-cost bound.
It uses the operator norm of $M$, not its spectral radius.
}

\textit{\DIFadd{Corollary 1 (continuous sensitivity before execution).}}  \DIFaddend Let
\DIFdelbegin \DIFdel{$\Delta_h=\|\zeta_h-\tilde\zeta_h\|$ }\DIFdelend \DIFaddbegin \DIFadd{$J_H^{\pi_c}(x_0,\zeta_0)$ }\DIFaddend and
\DIFdelbegin \DIFdel{$E_h=\|x_h-\tilde x_h\|$.  Lemma 2 and Assumption 1 give the one step bounds
}\begin{align*}
\DIFdel{\Delta_{h+1} }&\DIFdel{\le \|M\|\Delta_h+\|D\|L_dE_h, }\\
\DIFdel{E_{h+1} }&\DIFdel{\le L_z\Delta_h+L_xE_h . 
}\end{align*}%DIFAUXCMD
\DIFdel{For a fixed $\alpha>0$, define the weighted error
$V_h=\Delta_h+\alpha E_h$.
Multiplying \eqref{eq:state_step} by
$\alpha$ and adding \eqref{eq:delta_step} gives
}\begin{align*}
\DIFdel{V_{h+1}
...
\DIFdel{The two rollouts start }\DIFdelend \DIFaddbegin \DIFadd{$\tilde J_H^{\pi_c}(x_0,\tilde\zeta_0)$ be the predictive risks generated by
the continuous PC score and allocation layer }\DIFaddend from the same physical state \DIFdelbegin \DIFdel{, so $E_0=0$ and $V_0=\Delta_0\le\varepsilon$. Induction gives
$V_h\le\chi_{\alpha}^h\varepsilon$ for every $h$.
Since
$\Delta_h\le V_h$ and $E_h\le V_h/\alpha$, Assumption 2 yields
}\begin{displaymath}
...
\DIFdelbegin \DIFdel{Multiplying by $\gamma^h$ and summing from $h=0$ to $H-1$ gives
\eqref{eq:riskbound}.
The finite geometric series gives
the displayed closed
form when $\gamma\chi_{\alpha}<1$.  }%DIFDELCMD < \hfill %%%
\DIFdel{$\square$
}\DIFdelend \DIFaddbegin \DIFadd{For any $\alpha>0$, let
}\begin{equation}
\DIFadd{\bar\chi_{\alpha}=
\max\left\{\|M\|+\frac{\|R\|L_{\pi}^{z}}{\delta}+\alpha\bar L_z,\,
\bar L_x+\frac{\|R\|L_{\pi}^{x}}{\alpha\delta}\right\}.
}\end{equation}
\DIFadd{If $\|\zeta_0-\tilde\zeta_0\|\le\varepsilon$, then
}\begin{equation}
...
\DIFdelbegin %DIFDELCMD < \textit{%%%
\DIFdel{Assumption 3.}%DIFDELCMD < \MBLOCKRIGHTBRACE  %%%
\DIFdel{The transition,
stress,
stage risk, and PC rollout
action map are Lipschitz in the action argument on the compact feasible set.  There are constants $L_a$, $L_d^a$, $C_a$, $L_{\pi}^x$}\DIFdelend \DIFaddbegin \DIFadd{Here $L_\pi^x$ and $L_\pi^z$ are the constants for the continuous allocation
...
\textit{\DIFadd{Theorem 2 (conditional finite portfolio regret).}}  \DIFadd{Let $\mathcal P$ be the nonempty finite
...
$|Q(P)-\widehat Q(P)|\le\varepsilon_Q$ for every $P\in\mathcal P$}\DIFaddend , and \DIFdelbegin \DIFdel{$L_{\pi}^{z}$ such
that
}\begin{align*}
...
compositions of affine maps, bounded projections, }\DIFdelend \DIFaddbegin \DIFadd{Then
}\begin{equation}
...
$e(P)=Q(P)-\widehat Q(P)$ }\DIFaddend and
\DIFdelbegin \DIFdel{square roots on a compact
domain.  }\DIFdelend \DIFaddbegin \DIFadd{$\omega_Q=\max_{P\in\mathcal P}e(P)-\min_{P\in\mathcal P}e(P)$.
...
\DIFaddbegin \textit{\DIFadd{Proof.}}  \DIFadd{Let $P^*=\arg\min_P Q(P)$.  Uniform approximation and solver
tolerance give
...
\DIFaddend \textit{\DIFdelbegin \DIFdel{Corollary 1.}\DIFdelend \DIFaddbegin \DIFadd{Theorem 3 (conditional protection relative to an executable route).}\DIFaddend }
Let \DIFdelbegin \DIFdel{$J_H^{\pi}(x_0,\zeta_0)$ and $\tilde J_H^{\pi}(x_0,\tilde\zeta_0)$ be the
closed loop risks generated by PC
rollout from the same physical state and two initial threat estimates.
Define
}\begin{align*}
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{Q(P_g)-Q(P_b)\le\max\{0,\varepsilon_D-\mu-\tau\}.
\label{eq:protected-execution-bound}
}\end{equation}\DIFaddend 
\DIFaddbegin \DIFadd{In particular, $\varepsilon_D\le\mu+\tau$ is sufficient for noninferiority
...
\DIFdelbegin \textit{\DIFdel{Proof.}}  %DIFAUXCMD
\DIFdel{Let $a_h=\pi(x_h,\zeta_h,A)$ and $\tilde a_h=\pi(\tilde x_h,\tilde\zeta_h,A)$.  Assumption 3 }\DIFdelend \DIFaddbegin \textit{\DIFadd{Proof.}}  \DIFadd{The finite nonempty portfolio and finite scores give a
...
\DIFadd{\Delta(P_u,M)\le U(P_u)<-\mu-\tau.
}\]
\DIFadd{This proves \eqref{eq:protected-score-bound}.  Applying
\eqref{eq:differential-error-condition} at $P_u$ and $M_0$ }\DIFaddend gives
\DIFdelbegin \[
\DIFdel{\|a_h-\tilde a_h\|\le L_{\pi}^xE_h+L_{\pi}^{z}\Delta_h .
}\]%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
\DIFadd{Q(P_u)-Q(P_b)
}&\DIFadd{\le\Delta(P_u,M_0)+\varepsilon_D \nonumber}\\
&\DIFadd{\le U(P_u)+\varepsilon_D
<-\mu-\tau+\varepsilon_D.
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{Combining this bound with Assumptions 1 and
}\DIFdelend \DIFaddbegin \DIFadd{Combining this strict inequality with the zero difference in the fallback
branch proves \eqref{eq:protected-execution-bound}.  Under the absolute
error condition, define $e(P)=Q(P)-\widehat Q_t(P,M_0)$.  Then
}\[
\DIFadd{|Q(P)-Q(P_b)-\Delta(P,M_0)|
=|e(P)-e(P_b)|\le2\varepsilon_Q,
}\]
\DIFadd{which proves the final assertion.  }\hfill \DIFadd{$\square$
}

\DIFadd{The same matrix must be used in both terms of
...
\DIFadd{Theorem }\DIFaddend 3 \DIFdelbegin \DIFdel{yields
}\begin{align*}
\DIFdel{E_{h+1}}&\DIFdel{\le \bar L_xE_h+\bar L_z\Delta_h,}\\
\DIFdel{\Delta_{h+1}}&\DIFdel{\le
(\|M\|+\|D\|\bar L_d^z)\Delta_h+\|D\|\bar L_d^xE_h .
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{is an algebraic statement about the supplied finite scores.
...
\DIFadd{Returning $P_b$ means executing its route rather than merely assigning its
...
\DIFdelbegin \DIFdel{For $V_h=\Delta_h+\alpha E_h$, the preceding two inequalities give
$V_{h+1}\le\bar\chi_{\alpha}V_h$.  Since $V_0\le\varepsilon$, induction gives
$V_h\le\bar\chi_{\alpha}^h\varepsilon$.
The stage risk difference satisfies
}\[
\DIFdel{|L(x_h,a_h)-L(\tilde x_h,\tilde a_h)|
\le \bar C_xE_h+\bar C_z\Delta_h .
}\]%DIFAUXCMD
\DIFdel{Using $E_h\le V_h/\alpha$ }\DIFdelend \DIFaddbegin \DIFadd{The second line follows by induction, and the final line follows because it is a
convex combination of $q_{i,0}$ }\DIFaddend and \DIFdelbegin \DIFdel{$\Delta_h\le V_h$, then summing the
discounted
stage differences, gives the stated bound}\DIFdelend \DIFaddbegin \DIFadd{$\bar e/(1-\rho_q)$.  Thus backlog remains
...
well defined}\DIFaddend .  \hfill $\square$

Theorem 1 and Corollary 1 \DIFdelbegin \DIFdel{give a direct interpretation of the policy.
The
...
sensitivity.Restoration is evaluated through the future threat sequence that
enters the same finite horizon risk envelope}\DIFdelend \DIFaddbegin \DIFadd{concern finite horizon sensitivity in the continuous
...
\section{\DIFadd{Experimental Study}}
...
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{figure*}[t]\DIFnewtablecolor
\centering
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{table*}[t]\DIFnewtablecolor
\centering
...
The \DIFdelbegin \DIFdel{experiments use public yellow taxi trip records released by the New York
City Taxi and Limousine Commission to construct a large mobility driven charging load data set.  The complete year }\DIFdelend \DIFaddbegin \DIFadd{primary demand source is the City of Boulder Electric Vehicle Charging
...
for training, 2022 was used for model selection, and }\DIFaddend 2023 \DIFdelbegin \DIFdel{collection contains 38,310, 226 raw trip
records .  Invalid location identifiers,trips outside the calendar year,and
implausible trip distances are removed.  This leaves 36, 879,930 valid trips.  Trips are aggregated to hourly values over 263 zones, which gives 8760 time
steps.  For each hour and zone,the data construction computes pickup activity, drop off activity, trip mileage, charging energy demand, }\DIFdelend \DIFaddbegin \DIFadd{was used for final
...
\DIFadd{Power event evidence came from the CC BY 4.0 EAGLE-I release
...
\begin{table}[t]\DIFnewtablecolor
\centering
...
compared with 0.137 }\DIFaddend and \DIFdelbegin \DIFdel{origin destination
flows}\DIFdelend \DIFaddbegin \DIFadd{1.101 kWh for the historical mean.  Its energy RMSE
...
by its validation score, not by the smallest test error among these runs}\DIFaddend .

\DIFdelbegin \DIFdel{Let $d_{\tau i}^{\mathrm{mile}}$ denote the total trip distance assigned to zone $i$ in hour $\tau$.
Charging energy is estimated from this mileage by
}\begin{displaymath}
\DIFdel{e_{\tau i}
=\pi_{\mathrm{EV}}\epsilon_{\mathrm{mile}}d_{\tau i}^{\mathrm{mile}},
%DIFDELCMD < %%%
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $\pi_{\mathrm{EV}}=0.18$ is the assumed electric vehicle share and $\epsilon_{\mathrm{mile}}=0.24$ kWh per mile is the energy use coefficient for electric mobility.  }\DIFdelend The \DIFdelbegin \DIFdel{factor $\pi_{\mathrm{EV}}$ converts observed travel
...
graph and normalized.  Feeder capacity and communication capacity indicators are
deterministic functions of historical load intensity.  The resulting data set is
large, public, and directly reproducible in the digital twin.  }\DIFdelend \DIFaddbegin \DIFadd{spatial test pooled all five held out blocks and six horizons.
...
\end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Propagation calibration and policy comparison}}
\label{sec:policy-results}
\DIFaddend 

\DIFdelbegin \DIFdel{Table~\ref{tab:data} summarizes the quantities that enter the digital twin.  The
raw trip records are reduced to an hourly state tensor,
but the tensor still
preserves demand variation from a full year of city scale mobility records.  The charging and communication quantities remain fixed deterministic transformations
of the same processed activity data, which keeps the policy comparison tied to
one common operating environment}\DIFdelend \DIFaddbegin \begin{table}[t]\DIFnewtablecolor
\centering
...
persistence.  It is not the station propagation model used to construct $M$}\DIFaddend .
\DIFaddbegin \DIFadd{EAGLE-I estimated power persistence at 0.772 with a 90\% day block bootstrap interval
...
%DIFDELCMD < %%%
\DIFdel{The forecasting history is twelve hours and the rollout horizon is six hours.
The chronological data split uses the first part for training, the middle part
for validation, and the final part for
testing.  The graph recurrent predictor
is trained on a graphics processor.  The tested operating policies are static
allocation, greedy current state allocation, neural rollout,PC rollout, and an
oracle reference.  Static allocation uses historical averages.
Greedy
allocation reacts only to the current grid and charging state.  Neural rollout
...
matched }\DIFaddend uses the same \DIFdelbegin \DIFdel{forecast as PC rollout and assigns restoration from the current
threat field.  The oracle uses future realized demand
and threat values and is
included only as a reference .
}\DIFdelend \DIFaddbegin \DIFadd{objective, demand forecast, backlog update, completion
...
small perturbations.
The single domain group perturbs one of mobility,
power,or communication.  The cascading group allows cross domain }\DIFdelend \DIFaddbegin \DIFadd{The service score was compared with exposure scoring on 512 validation
...
\DIFadd{The test comprised 4,096 paired scenarios with 1,024 per threat class.
...
PC with reference protection & Common forecast & Current threat and backlog & Yes & Paired difference over the same 21 matrices & No & Replace matched route only under the fixed acceptance rule & Yes \\
...
PC with reference protection & 1112.57 & 0.0993 \\
...
service score without }\DIFaddend propagation.  The \DIFdelbegin \DIFdel{unseen compound group uses stronger and longer disruptions than those used in
the nominal and single domain cases.  Each group contains the same number of
scenarios.  Every policy is evaluated on the same realized demand and
threat
sequence}\DIFdelend \DIFaddbegin \DIFadd{paired difference was $-1.1436$ with
...
proportional to historical demand.
This makes disruptions more likely to enter
...
\DIFdel{where $\kappa_0$ is temporal persistence, $\rho_g$ controls spatial spread,
}\DIFdelend \DIFaddbegin \DIFadd{larger effect came from evaluating service rather than exposure alone.
...
$[-8.3843,-7.0077]$ }\DIFaddend and \DIFdelbegin \DIFdel{$\nu_h$ is a
bounded multiplicative perturbation.  Mobility, power, and communication threat fields are obtained by domain specific scaling of $b_h$.
In cascading and unseen compound scenarios,cross domain terms are added before
projection,
}\begin{align*}
...
\DIFdel{The coefficients $\tau_{pc}$, $\tau_{rc}$, $\tau_{cr}$,
}\DIFdelend \DIFaddbegin \DIFadd{$[-13.1616,-11.0704]$.  Both overall comparisons remained
...
\DIFadd{Robust PC had mean cost 1112.90.  Its difference from central PC was 0.3670,
with marginal interval $[0.0788,0.6627]$.  Its upper 5\% and 1\% conditional
mean costs exceeded central PC by 2.85 }\DIFaddend and \DIFdelbegin \DIFdel{$\tau_{cp}$ specify
power to communication, mobility to communication, communication to mobility,
...
the realized state and applies its own rollout rule.  The shared scenariostream
ensures that differences in Table~\ref{tab:rollout} come from policy decisions
rather than from different disturbance draws.
}\DIFdelend \DIFaddbegin \DIFadd{4.73, respectively, while the maximum
...
charging demand }\DIFdelend \DIFaddbegin \begin{table}[t]\DIFnewtablecolor
\centering
...
\begin{figure*}[t]\DIFnewtablecolor
\centering
...
beside the electrical, communication}\DIFaddend , \DIFdelbegin \DIFdel{communication loss, feeder margin violation, }\DIFdelend and \DIFdelbegin \DIFdel{cascade
interaction terms appearing in \eqref{eq:stage}.  Service continuity is computed
as served
mobility divided by total
mobility over the horizon.
The final tables
...
}\DIFdelend \DIFaddbegin \DIFadd{crew execution results.  The panels
...
\DIFdelbegin \DIFdel{Table~\ref{tab:policies} clarifies the main comparison.
Neural rollout and PC
rollout have the same forecast input.  Their difference is the restoration
allocation.
This design keeps the experiment aligned with the analysis in
Theorem 1 and
Corollary }\DIFdelend \DIFaddbegin \begin{table}[t]\DIFnewtablecolor
\centering
...
\DIFadd{Table~\ref{tab:integrated-execution} summarizes the feeder and crew outcomes in
...
using }\DIFaddend 1,\DIFdelbegin \DIFdel{where the performance gap is connected to future
threat growth rather than to a different demand estimator.  }\DIFdelend \DIFaddbegin \DIFadd{024 paired scenarios and six policies in the same loop.  It produced
...
The \DIFdelbegin \DIFdel{experiments require two types of computation.  The graph forecasting model
...
scenarios and an additional set of power oriented stress cases.
Reduced
...
graphics and central processor resources.  }\DIFdelend \DIFaddbegin \DIFadd{packet emulator is a finite buffer queue with first come, first served
...
\DIFdelbegin \DIFdel{The computation settings in }\DIFdelend Table~\DIFdelbegin \DIFdel{\ref{tab:compute} explain why the evaluation was run as a
repeated parallel experiment rather than as a small example.
The
forecasting model is modest in size, while the
evaluation requires repeated
...
processor cores.  }\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:packet-feedback} reports the effect of backup duration.
...
\end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{In the integrated primary run, the route generated for four crews was not a diagnostic added
...
\end{table*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{tab:route-crew} reports the route comparison for every crew and
...
\DIFdelbegin \DIFdel{Figure~\ref{fig:system} fixes the order in which the numerical evidence should
be read.
The mobility and charging state enters the graph predictor, the threat
field enters the monotone propagation model }\DIFdelend \DIFaddbegin \DIFadd{The integrated sensitivity study holds the realized coefficient distribution
fixed while perturbing the matrix available to the controller.  Each condition
uses the same 1}\DIFaddend ,\DIFaddbegin \DIFadd{024 scenario inputs.  With $0.75M$, central PC exceeded matched
...
$[-2.6349,-1.1321]$.  Robust PC exceeded central PC by 3.4569 }\DIFaddend and \DIFdelbegin \DIFdel{both streams are evaluated by
the digital twin before the first restoration action is applied.  }\DIFdelend \DIFaddbegin \DIFadd{1.2651 in
...
\DIFadd{A separate test changed the form of the realized transition rather than its
...
\subsection{\DIFadd{Reference protection and independent charging input}}
\label{sec:reference-protection}

\DIFaddend The \DIFdelbegin \DIFdel{forecast
errors }\DIFdelend \DIFaddbegin \DIFadd{matrix error comparisons motivate protection relative to a common
...
\DIFadd{Let $E^+(P)=n^{-1}\sum_i[Q_i(P)-Q_i(P_b)]_+$, where each $P_b$ is the
...
\DIFadd{On the original routes, reference protection retained a mean cost of
...
\DIFadd{Across the six error conditions, the paired decrease in positive excess was
...
comparisons are retained }\DIFaddend in Table~\DIFdelbegin \DIFdel{\ref{tab:forecast} therefore serve as a
common input check
rather than as an additional policy choice.  Historical mean }\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:guarded-tails}.
}

\DIFadd{The separate parameter-independent route construction tests whether a
...
\DIFadd{An independent geographic input used the City of Palo Alto charging
records.}\footnote{\DIFadd{City of Palo Alto, Electric Vehicle Charging Station Usage,
published February 2021. The source record is available at
}\url{https://data.paloalto.gov/dataviews/257812/electric-vehicle-charging-station-usage-july-2011-dec-2020/}\DIFadd{.}}
\DIFadd{The complete municipal source contained 259,415 sessions.  Fixed quality
rules and a requirement of at least 100 valid sessions in 2017 }\DIFaddend and \DIFdelbegin \DIFdel{persistence
miss much of the calendar regularity in urban travel.
The weekly seasonal rule
is stronger because taxi demand has a visible weekly rhythm,}\DIFdelend \DIFaddbegin \DIFadd{2018
...
and budgets used 2017 }\DIFaddend and \DIFdelbegin \DIFdel{the graph
...
PC rollout
then inherit the same demand estimate, so the later comparison isolates the effect of the restoration rule.  }\DIFdelend \DIFaddbegin \DIFadd{2018 only.  The charging, communication, and
...
\end{table*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The restoration parameters and selected margin were unchanged.  One demand
...
\end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Each input used 1,024 paired scenarios per condition, with all eight
...
many scenariosare nominal or single domain, and
most policies can still keep a large share of mobility service available.  Total costseparates the
methods
...
Greedy
allocation reacts to present stress
without modeling threat entering from
neighboring zones.
Neural rollout uses future demand for service placement, and
PC rollout keeps that demand information while changing the restoration side of
the decision.
The lower cost is
therefore tied to the future cascade envelope
in \eqref{eq:envelope}, not to a stronger predictor.  }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Numerical and mathematical checks}}
\label{sec:checks}

\DIFadd{The continuous service cost and the executor were compared in 24 cases with
...
\DIFadd{Electrical replay checks covered six load scales, including three infeasible
...
\DIFadd{The added protection study retained 302,592 policy records, including every
...
The \DIFdelbegin \DIFdel{class split }\DIFdelend \DIFaddbegin \DIFadd{robust extension provides a separate comparison with central PC.  Its
...
all tested errors in the controller's matrix.  The scoring choices }\DIFaddend in
Table~\DIFdelbegin \DIFdel{\ref{tab:threat-class} gives the
mechanism behind the average result}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:policy-definitions} therefore distinguish service cost reduction,
propagation benefit, and conditional protection against model error.
}

\DIFadd{Reference protection addresses a different comparison.  It accepts a route
...
\DIFadd{The public data sources enter through distinct variables.  Boulder
...
\DIFadd{The Appendix, part~\ref{supp:theory}, contains the complete continuous
...
}\url{https://github.com/ahshicr/TSG-01697-2026-reproducibility}\DIFaddend .
\DIFdelbegin \DIFdel{Nominal cases leave little room for a future propagation model
to help, }\DIFdelend \DIFaddbegin 
...
\subsection{\DIFadd{Continuous operators and complete sensitivity proofs}}
\label{supp:theory}

\DIFadd{The Lipschitz results below concern the continuous prediction and allocation
...
\DIFadd{The analysis uses the Euclidean norm.  The projection onto a closed convex set
is nonexpansive, so
}\begin{equation}
\DIFadd{\|\Pi_{\mathcal{Z}}(u)-\Pi_{\mathcal{Z}}(v)\|\le \|u-v\|.
\label{si:eq:projection}
}\end{equation}

\textit{\DIFadd{Condition 1.}}  \DIFadd{On the stated bounded continuous prediction domain and
for all feasible requested actions, the one step service state operator
$F_c(x,a,\zeta)$ is
Lipschitz in $(x,\zeta)$ with finite constants $L_x$ }\DIFaddend and \DIFdelbegin \DIFdel{single domain cases are largely handled by current state reaction.  The gap widens in cascading and unseen compound cases, where a zone may have
moderate current loss but high future exposure because it lies between a power
disturbance and a communication sensitive mobility corridor.
This operating
...
transmission point.  }\DIFdelend \DIFaddbegin \DIFadd{$L_z$.
}\begin{equation}
...
\end{table*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{Condition 2.}}  \DIFadd{The continuous stage risk $L_c$ is Lipschitz in
$(x,\zeta)$ with constants $C_x$ and $C_z$ for all feasible actions.
}\begin{equation}
\DIFadd{|L_c(x,a,\zeta)-L_c(\tilde x,a,\tilde\zeta)|
\le C_x\|x-\tilde x\|+C_z\|\zeta-\tilde \zeta\|.
}\end{equation}
\textit{\DIFadd{Lemma 2.}}  \DIFadd{Conditions 1 and 2 hold for the continuous service equations
...
\end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{Proof.}}  \DIFadd{Let $\mathcal D$ denote the product of the closed state,
...
%DIFDELCMD < %%%
\DIFdel{The grid stress cases extend the comparison beyond average cascade behavior.
In Table~\ref{tab:smartgrid-stress} and Fig.  ~\ref{fig:smartgrid-stress}, charging demand is increased and feeder margin is
reduced, so feeder violation
and unserved charging both rise with stress level.
PC rollout remains below
neural rollout in cost across these cases, although the relative gain becomes
smaller under the most severe feeder stress.  That change is important for interpreting the result.
Restoration can reduce propagation through the communication and mobility layers,
but it cannot create feeder capacity removed
by the stress setting.  The coupling cases give the complementary view.  Stronger
power to communication and communication to mobility coupling raises total cost }\DIFdelend \DIFaddbegin \DIFadd{For bounded scalars $a,a',b,b'$, the product satisfies
}\begin{equation}
...
$\bar m$}\DIFaddend , \DIFdelbegin \DIFdel{while the PC rollout gain remains present because the envelope still shifts
restoration toward zones likely to transmit disturbance across domains.  }\DIFdelend \DIFaddbegin \DIFadd{$\bar e$, and $\bar q$, and set $\bar b=\bar e+\rho_q\bar q$ and
...
\DIFdelbegin \DIFdel{Changing the restoration budget in
Table~\ref{tab:restore-sensitivity} adds a
second check on the same mechanism.  A larger budget lowers cost and
improves
service continuity, while feeder violation changes only moderately because
local capacity and exogenous charging demand still constrain that term.  The
largest benefit comes from reducing communication loss and
mobility delay during
cascading disturbances.  This ordering matches Theorem 1 and Corollary }\DIFdelend \DIFaddbegin \textit{\DIFadd{Lemma 3.}}  \DIFadd{For fixed action sequence and disturbance sequence, if two
...
\textit{\DIFadd{Proof.}}  \DIFadd{Let $\Delta_h=\|\zeta_h-\tilde\zeta_h\|$.
...
\textit{\DIFadd{Lemma 4.}}  \DIFadd{Under Condition }\DIFaddend 1\DIFdelbegin \DIFdel{, which
identify propagation growth, stress feedback, }\DIFdelend \DIFaddbegin \DIFadd{, the service state error satisfies
}\begin{equation}
\DIFadd{E_h\le
\sum_{k=0}^{h-1}L_x^{h-1-k}L_z\Delta_k .
\label{si:eq:lemma2}
...
\textit{\DIFadd{Proof.}}  \DIFadd{The two rollouts use the same action sequence.  Condition 1
gives $E_{h+1}\le L_xE_h+L_z\Delta_h$.  Since the initial physical state is the
same, $E_0=0$.  Expanding the recursion gives
$E_1\le L_z\Delta_0$ and
$E_2\le L_xL_z\Delta_0+L_z\Delta_1$.  Repeating the expansion gives
\eqref{si:eq:lemma2}.  }\hfill \DIFadd{$\square$
}

\textit{\DIFadd{Theorem 1.}}  \DIFadd{Let $J_H(x_0,\zeta_0)$ be the discounted $H$ step sum of $L_c$ for
a fixed feasible action sequence, }\DIFaddend and \DIFdelbegin \DIFdel{action sensitivity as the quantities that control finite horizon risk }\DIFdelend \DIFaddbegin \DIFadd{let $\tilde J_H(x_0,\tilde\zeta_0)$ be the
corresponding risk for an initial threat estimate $\tilde\zeta_0$.  For any
$\alpha>0$, define
}\begin{equation}
\DIFadd{\chi_{\alpha}=
...
\DIFadd{|J_H(x_0,\zeta_0)-\tilde J_H(x_0,\tilde\zeta_0)|
\le
\varepsilon \left(C_z+\frac{C_x}{\alpha}\right)
\sum_{h=0}^{H-1}(\gamma\chi_{\alpha})^h .
\label{si:eq:riskbound}
}\end{equation}
\DIFadd{For every finite $H$ the sum is finite.  If $\gamma\chi_{\alpha}<1$, it is also
uniformly bounded as the horizon increases by
}\begin{equation}
\DIFadd{\varepsilon \left(C_z+\frac{C_x}{\alpha}\right)
\frac{1}{1-\gamma\chi_{\alpha}}.
...
\textit{\DIFadd{Proof.}}  \DIFadd{Let
$\Delta_h=\|\zeta_h-\tilde\zeta_h\|$ and
$E_h=\|x_h-\tilde x_h\|$.  Lemma 3 and Condition 1 give the one step bounds
}\begin{align}
\DIFadd{\Delta_{h+1} }&\DIFadd{\le \|M\|\Delta_h, \label{si:eq:delta_step}}\\
\DIFadd{E_{h+1} }&\DIFadd{\le L_z\Delta_h+L_xE_h . \label{si:eq:state_step}
}\end{align}
\DIFadd{For a fixed $\alpha>0$, define the weighted error
...
\DIFadd{The two rollouts start from the same physical state, so $E_0=0$ and
...
\DIFadd{Multiplying by $\gamma^h$ and summing from $h=0$ to $H-1$ gives
\eqref{si:eq:riskbound}.  The infinite geometric series gives the uniform bound
when $\gamma\chi_{\alpha}<1$}\DIFaddend .  \DIFdelbegin \DIFdel{PC rollout does not alter the demand forecast used by neural rollout.  It changes restoration so that
future
cross domainexposure is smaller before the cascade becomes visible in the
current loss map.  }\DIFdelend \DIFaddbegin \hfill \DIFadd{$\square$
}\DIFaddend 

\DIFdelbegin \section{\DIFdel{Conclusion}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textit{\DIFadd{Condition 3.}}  \DIFadd{The continuous service transition and stage risk are
...
\textit{\DIFadd{Lemma 5.}}  \DIFadd{Condition 3 holds for the continuous PC map on the stated
compact domain.
}\DIFaddend 
...
forecasting, a physics guided digital twin, a monotone threat propagation model,
}\DIFdelend \DIFaddbegin \textit{\DIFadd{Proof.}}  \DIFadd{Lemma 2 already establishes the first two inequalities in the
...
\DIFdelbegin \DIFdel{a concave restoration allocation rule.  The theoretical analysis gives
a
finite horizon risk bound that connects rollout performance to the propagation
growth factor.  The public city scale mobility experiment indicates that the method is most useful in cascading, compound, }\DIFdelend \DIFaddbegin \DIFadd{$|S(v)-S(w)|\le\sqrt N\|r(v)-r(w)\|_2$.  Since
...
tangent have derivative bounds $1/4$ }\DIFaddend and \DIFdelbegin \DIFdel{stressed charging operation,
while normal
operation remains close to the neural rollout baseline.  Full year
training }\DIFdelend \DIFaddbegin \DIFadd{$1$.  The GRU combines these maps with
...
\DIFadd{At $v=0$, the bracket is $s^{-1}I$.  Otherwise, it has eigenvalues $s^{-1}$ perpendicular to $v$ }\DIFaddend and
\DIFdelbegin \DIFdel{repeated closed loop scenario evaluation benefit from parallel
graphics and central processor resources.
}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_{\rm LN}/s^3$ along $v$.  Both are at most
...
\DIFaddbegin \DIFadd{Exact GELU is $t\Phi(t)$, where $\Phi$ and $\phi$ are the standard normal
...
\DIFadd{The forecast exposure weights use positive fixed training means, sums, products,
...
\textit{\DIFadd{Corollary 1 (continuous sensitivity before execution).}}  \DIFadd{Let
...
\textit{\DIFadd{Proof.}}  \DIFadd{Let $a_h=\pi_c(x_h,\zeta_h,A)$ and
$\tilde a_h=\pi_c(\tilde x_h,\tilde\zeta_h,A)$.  Condition 3 gives
}\[
\DIFadd{\|a_h-\tilde a_h\|\le L_{\pi}^xE_h+L_{\pi}^{z}\Delta_h .
}\]
\DIFadd{Combining this bound with Conditions 1 and 3 and the restoration term in
...
\DIFadd{For $V_h=\Delta_h+\alpha E_h$, the preceding two inequalities give
$V_{h+1}\le\bar\chi_{\alpha}V_h$.  Since $V_0\le\varepsilon$, induction gives
...
\DIFadd{Using $E_h\le V_h/\alpha$ and $\Delta_h\le V_h$, then summing the discounted
stage differences, gives the stated bound.  }\hfill \DIFadd{$\square$
}


\subsection{\DIFadd{Parameters and additional comparisons}}
\suppressfloats[t]
...
\DIFadd{The added reference-protection study uses the same numerical settings unless
...
\DIFappendixpage
\begin{table*}[p]\DIFnewtablecolor
\centering
...
\DIFdel{S.  ~Cui, Y.  ~Wang, J.  ~Xiao, and N.  ~Wang, ``Optimal pricing of public electric
  vehicle charging stations considering operations of coupled transportation
  and power systems, '' }\emph{\DIFdel{IEEE Transactions on Smart Grid}}%DIFAUXCMD
\DIFdel{, vol.~12, no. ~4, pp. 3278--3288, 2021.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{ran2024fast}
\DIFdel{L.
~Ran,
J.~Qin, Y. ~Wan, W.
~Fu, W.~Yu, and }\DIFdelend \DIFaddbegin \bibitem{he2015charging}
\DIFaddend F.~\DIFdelbegin \DIFdel{Xiao, ``Fast charging navigation
  strategy of evs in power-transportation networks: A coupled network weighted
  pricing perspective,'' }\DIFdelend \DIFaddbegin \DIFadd{He, Y.~Yin, and J.~Zhou, ``Deploying public charging stations for electric
  ...
  Emerging Technologies}\DIFaddend }, vol.~\DIFdelbegin \DIFdel{15, no.~4, pp. 3864--3875, 2024.
}\DIFdelend \DIFaddbegin \DIFadd{60, pp. 227--240, 2015.
}\DIFaddend 

\bibitem{wei2018trafficpower}
W.~Wei, \DIFdelbegin \DIFdel{D.~Wu, J.~Wang, and }\DIFdelend S.~\DIFdelbegin \DIFdel{Mei}\DIFdelend \DIFaddbegin \DIFadd{Mei, L.~Wu, M.~Shahidehpour, and Y.~Fang}\DIFaddend , ``Optimal traffic-power
  flow in urban electrified transportation networks,'' \emph{IEEE Transactions
  on Smart Grid}, vol.~8, no.~1, pp. 84--95, 2017.

\bibitem{qian2021fast}
\DIFdelbegin \DIFdel{K.~Qian, }\DIFdelend C.~\DIFdelbegin \DIFdel{Zhou, M.~Allan, and }\DIFdelend \DIFaddbegin \DIFadd{Shao, T.~Qian, }\DIFaddend Y.~\DIFdelbegin \DIFdel{Yuan}\DIFdelend \DIFaddbegin \DIFadd{Wang, and X.~Wang}\DIFaddend , ``Coordinated planning of extreme fast
  charging stations and power distribution networks considering on-site
  storage,'' \emph{IEEE Transactions on Intelligent Transportation Systems},
  vol.~22, no.~1, pp. 493--504, 2021.

\DIFaddbegin \bibitem{cui2021pricing}
\DIFadd{Y.~Cui, Z.~Hu, and X.~Duan, ``Optimal pricing of public electric vehicle
  charging stations considering operations of coupled transportation and power
  systems,'' }\emph{\DIFadd{IEEE Transactions on Smart Grid}}\DIFadd{, vol.~12, no.~4, pp.
  3278--3288, 2021.
}

\DIFaddend \bibitem{li2024inverse}
...
\DIFdel{Z.~Wu, S.~Pan, G.~Long, J.~Jiang, and C.~Zhang, ``Graph wavenet for deep
  ...
  Joint Conference }\DIFdelend \DIFaddbegin \DIFadd{IEEE Transactions }\DIFaddend on \DIFdelbegin \DIFdel{Artificial Intelligence}\DIFdelend \DIFaddbegin \DIFadd{Power Systems}\DIFaddend }, \DIFdelbegin \DIFdel{2019, pp. 1907--1913.
}\DIFdelend \DIFaddbegin \DIFadd{vol.~38, no.~1, pp. 872--884,
  2023.
}\DIFaddend 

\DIFdelbegin \bibitem{old-deng2021stnorm}
\DIFdelend \DIFaddbegin \bibitem{zhang2023datasecurity}
\DIFadd{Z.~Zhang, Z.~Yang, D.~K.~Y. Yau, Y.~Tian, and }\DIFaddend J.~\DIFdelbegin \DIFdel{Deng, X.~Chen, R.~Jiang, X.~Song, and I.~W. Tsang, ``St-norm: Spatial and
  temporal normalization for multi-variate time series forecasting,'' in
  }%DIFDELCMD < \emph{%%%
\DIFdel{Proceedings of }\DIFdelend \DIFaddbegin \DIFadd{Ma, ``Data security of machine
  learning applied in low-carbon smart grid: A formal model for }\DIFaddend the
  \DIFdelbegin \DIFdel{ACM SIGKDD Conference on Knowledge Discovery and
  Data Mining}\DIFdelend \DIFaddbegin \DIFadd{physics-constrained robustness,'' }\emph{\DIFadd{Applied Energy}\DIFaddend }, \DIFdelbegin \DIFdel{2021, pp. 269--278.
}\DIFdelend \DIFaddbegin \DIFadd{vol. 347, p. 121405,
  2023.
}\DIFaddend 

\DIFdelbegin \bibitem{old-jiang2023staeformer}
\DIFdel{H.~Liu, }\DIFdelend \DIFaddbegin \bibitem{zhang2026physicsattack}
\DIFaddend Z.~\DIFdelbegin \DIFdel{Dong, R.~Jiang, J.~Deng}\DIFdelend \DIFaddbegin \DIFadd{Zhang, W.~Wang, M.~Wang, H.~Wang}\DIFaddend , J.~\DIFdelbegin \DIFdel{Deng, Q.~Chen, and X.~Song, ``Spatio-temporal adaptive embedding makes vanilla transformer sota for traffic forecasting,'' in }\DIFdelend \DIFaddbegin \DIFadd{Bi, and G.~Xu, ``Physics-constrained
  adversarial attack generation and active defense for the hybrid deep
  reinforcement learning-based load frequency control,'' }\DIFaddend \emph{\DIFdelbegin \DIFdel{Proceedings of the }\DIFdelend ACM \DIFdelbegin \DIFdel{International
  Conference }\DIFdelend \DIFaddbegin \DIFadd{Transactions
  }\DIFaddend on \DIFdelbegin \DIFdel{Information }\DIFdelend \DIFaddbegin \DIFadd{Autonomous }\DIFaddend and \DIFdelbegin \DIFdel{Knowledge Management}\DIFdelend \DIFaddbegin \DIFadd{Adaptive Systems}\DIFaddend }, \DIFdelbegin \DIFdel{2023, pp. 4125--4129.
}\DIFdelend \DIFaddbegin \DIFadd{2026, advance online publication, March
  28, 2026.
}\DIFaddend 

\DIFdelbegin \bibitem{old-zhang2023megacrn}
\DIFdel{R.~Jiang, Z.~Wang, J.~Yong, P.~Jeph, Q.~Chen, Y. ~Kobayashi}\DIFdelend \DIFaddbegin \bibitem{ding2021digital}
\DIFadd{K.~Ding, F.~T.~S. Chan}\DIFaddend , X.~\DIFdelbegin \DIFdel{Song, S.~Fukushima, and T.~Suzumura, ``Spatio-temporal meta-graph learning for traffic forecasting,'' in }\DIFdelend \DIFaddbegin \DIFadd{Zhang, G.~Zhou, and F.~Zhang, ``Defining a digital
  twin-based cyber-physical production system for autonomous manufacturing in
  smart shop floors,'' }\DIFaddend \emph{\DIFdelbegin \DIFdel{Proceedings }\DIFdelend \DIFaddbegin \DIFadd{International Journal }\DIFaddend of \DIFdelbegin \DIFdel{the AAAI Conference on
  Artificial Intelligence}\DIFdelend \DIFaddbegin \DIFadd{Production Research}\DIFaddend },
  vol.~\DIFdelbegin \DIFdel{37, 2023, pp.8078--8086.
}\DIFdelend \DIFaddbegin \DIFadd{57, no.~20, pp. 6315--6334, 2019.
}\DIFaddend 

\DIFdelbegin \bibitem{old-yin2022deep}
\DIFdel{X.~Yin, G. ~Wu, J.~Wei, }\DIFdelend \DIFaddbegin \bibitem{petrik2016baseline}
\BIBentryALTinterwordspacing
\DIFadd{M.~Petrik, M.~Ghavamzadeh, and }\DIFaddend Y.~\DIFdelbegin \DIFdel{Shen, H.~Qi, and B.~Yin, ``Deep learning on traffic
  prediction: Methods,analysis, and future directions,'' }\DIFdelend \DIFaddbegin \DIFadd{Chow, ``Safe policy improvement by minimizing
  robust baseline regret,'' in }\DIFaddend \emph{\DIFdelbegin \DIFdel{IEEE
  Transactions on Intelligent Transportation }\DIFdelend \DIFaddbegin \DIFadd{Advances in Neural Information Processing
  }\DIFaddend Systems}, vol.~\DIFdelbegin \DIFdel{23,
  no.~6, pp.
  4927--4943}\DIFdelend \DIFaddbegin \DIFadd{29.}\hskip 1em plus 0.5em minus 0.4em\relax \DIFadd{Curran Associates,
  Inc.}\DIFaddend , \DIFdelbegin \DIFdel{2022.
}\DIFdelend \DIFaddbegin \DIFadd{2016, pp. 2298--2306. }[\DIFadd{Online}]\DIFadd{. Available:
  }\url{https://proceedings.neurips.cc/paper_files/paper/2016/hash/9a3d458322d70046f63dfd8b0153ece4-Abstract.html}
