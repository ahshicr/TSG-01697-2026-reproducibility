...
def evaluate_task(task):
    ...
    for name, plan in plans.items():
        score = sim.projected_route_loss(initial, forecast, plan, sim.G["central_policy_coefficients"])
        ...
        rows.append({"mode": mode, "scenario_id": scenario_id, "group": group,
            "candidate": name, "predicted_score": score, "realized_cost": metrics["cost"],
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("results/submission_20260905/route_error"))
